...
def drop_files_if_exists(file_path: str):
...
def upload_to_gcs_with_progress(bucket: str, object_name: str, local_file: str):
    ...
    drop_files_if_exists(local_file)


def web_to_gcs(year, service, file):
    client = storage.Client()
    bucket_obj = client.bucket(BUCKET)
    ...
    for i in tqdm(range(12), desc=f"{service} {year}", unit="month"):
        ...
        csv_file_name = f"data/temp/{service}_tripdata_{year}-{month}.csv.gz"
        parquet_file_name = csv_file_name.replace(".csv.gz", ".parquet")
        object_name = f"{file}/{service}/{parquet_file_name.replace('data/temp/', '')}"
        ...
        if os.path.exists(csv_file_name):
            ...
            request_url = f"{init_url}{service}/{csv_file_name.replace('data/temp/', '')}"
...
if __name__ == "__main__":
    ...
    # web_to_gcs("2019", "green", "taxis")
    # web_to_gcs("2020", "green", "taxis")
    # web_to_gcs("2021", "green", "taxis")  # will fail when reaching 08 (normal, file does not exists in github :)
    web_to_gcs("2019", "yellow", "taxis")
    web_to_gcs("2020", "yellow", "taxis")
    web_to_gcs("2021", "yellow", "taxis") # will fail when reaching 08 (normal, file does not exists in github :)